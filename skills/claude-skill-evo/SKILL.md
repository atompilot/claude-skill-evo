...
description: >
  ...
  审查 skill、skill review、检查 skill、skill 质量、review skill。
version: 3.1.0
...
/skill-evo review                       # 审查所有 SKILL.md 质量
...
```
Phase 0: 现状感知（判断模式）
    ├─ review 模式 → Phase R: Skill 质量审查（独立路径，不走 1-5）
    └─ 其他模式 ↓
...
| $ARGUMENTS | 模式 | 行为 |
|-----------|------|------|
| 包含 `review` 或 `审查` 或 `检查 skill` | **审查模式** | 直接跳到 Phase R，不走 Phase 1-5 |

若 $ARGUMENTS 不含 review/审查，再根据文件现状判断：
...
> 仅当 $ARGUMENTS 包含 `review`/`审查`/`检查 skill` 时进入。执行完毕后直接结束，不走 Phase 1-5。
...
- 若 $ARGUMENTS 指定了 skill 名（如 `review {prefix}-dev`）→ 只审查该 skill
- 否则 → 审查所有 SKILL.md
